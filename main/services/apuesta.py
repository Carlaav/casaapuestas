...
class CuotaStrategy(ABC):
    # VAMOS A UTILIZAR UNA FORMULA GENERAL.
    def calcular_cuota(self, cuota):
        """Calcular probabilidad"""

    def puntos_totales(self, couta):
        puntos_local = couta.partido.equipo_local.puntaje
        puntos_visitante = couta.partido.equipo_visitante.puntaje
        minimo = min(puntos_local, puntos_visitante)
        return puntos_local + puntos_visitante + minimo

    def calcular_couta_final(self, probabilidad):
        couta = 1 / probabilidad
        couta = couta / 1.25
        return couta
...
class CuotaLocal(CuotaStrategy):
    def calcular_cuota(self, cuota):
        probabilidad = cuota.partido.equipo_local.puntaje / self.puntos_totales(cuota)
        return self.calcular_couta_final(probabilidad)


class CuotaVisitante(CuotaStrategy):
    def calcular_cuota(self, cuota):
        probabilidad = cuota.partido.equipo_visitante.puntaje / self.puntos_totales(cuota)
        return self.calcular_couta_final(probabilidad)


class CuotaEmpate(CuotaStrategy):
    def calcular_cuota(self, cuota):
        probabilidad = (cuota.partido.equipo_visitante.puntaje + cuota.partido.equipo_local.puntaje) / self.puntos_totales(cuota)
